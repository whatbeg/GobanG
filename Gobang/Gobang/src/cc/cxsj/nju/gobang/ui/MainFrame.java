...
public class MainFrame extends JFrame {
	...
	private void fillChessBoard(int stepNum) {
        ...
        Font font = new Font("MS Song", Font.BOLD, 26);
        if (step.isValidStep) {
			for (int r = 0; r < ROWS; r++) {
				for (int c = 0; c < COLS; c++) {
					// chessBoard[r][c].setText(step.board[r][c]);
                    // chessBoard[r][c].setFont(font);
                    // chessBoard[r][c].setHorizontalAlignment(JTextField.CENTER);
                    // chessBoard[r][c].setAlignmentY(Double.valueOf(0).floatValue());
	...
	public void updateChessBoardUI(int r, int c, int color) {
